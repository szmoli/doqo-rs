use std::{collections::HashMap, fs::{self, File}, path::PathBuf, rc::Rc};

use serde::{Deserialize, Serialize};
use ts_rs::TS;

use crate::{LanguagePlugin, Symbol, SymbolId, plugin::PluginId, source::{SourceId, Source}};
...
pub struct Registry {
  ...
  pub sources: HashMap<SourceId, Rc<Source>>,
  #[serde(skip)]
  #[ts(skip)]
  pub plugins: HashMap<PluginId, Rc<Box<dyn LanguagePlugin>>>,
...
impl Registry {
    ...
    pub fn register_source(&mut self, path: PathBuf, plugin_id: PluginId) -> SourceId {
      ...
      let friendly_language_name = self.plugins.get(&plugin_id)
